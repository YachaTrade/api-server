...
mod tests {
    ...
    async fn gift_fee_union_lp_only_sorts_last(pool: PgPool) {
        ...
        sqlx::query(r#"INSERT INTO token (token_id,name,symbol,image_uri,creator,description,is_nsfw,is_graduated,is_cto,created_at,transaction_hash,total_supply) VALUES ($1,'Gift','GFT','',$2,NULL,false,false,false,100,'0xh',1000000) ON CONFLICT DO NOTHING"#).bind(GIFT_TOKEN).bind(OTHER).execute(&pool).await.unwrap();
        // LP_TOKEN: created by OTHER, ACCOUNT provides LP, NOT a gift receiver
        sqlx::query(r#"INSERT INTO token (token_id,name,symbol,image_uri,creator,description,is_nsfw,is_graduated,is_cto,created_at,transaction_hash,total_supply) VALUES ($1,'Lp','LP','',$2,NULL,false,false,false,200,'0xh',1000000) ON CONFLICT DO NOTHING"#).bind(LP_TOKEN).bind(OTHER).execute(&pool).await.unwrap();
