...
/// 2) Redis positive cache hit → 통과 (1주일 TTL)
/// 3) cache miss → `SELECT EXISTS(SELECT 1 FROM token WHERE token_id = $1)`
///    - exists=true → positive cache 저장 후 통과
///    - exists=false → **404 NotFound**
...
pub async fn valid_existing_token_id(state: &AppState, token_id: &str) -> Result<String, AppError> {
    ...
    match exists_result {
        Ok(true) => {
            if let Err(error) = state.redis.set_token_exists(&token_id).await {
                warn!("set_token_exists failed for {}: {}", token_id, error);
            }
            Ok(token_id)
        }
