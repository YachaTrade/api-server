...
impl TokenManagementController {
    ...
    pub async fn get_dev_positions(
        ...
        let tokens_future = sqlx::query_as::<_, DevPositionRaw>(&query)
            .bind(account_id)
        ...
        let count_future = sqlx::query(count_query)
            .bind(account_id)
    ...
    pub async fn get_holding_token_management(
        ...
        let rows = sqlx::query_as::<_, HoldingTokenManagementRaw>(&query)
            .bind(account_id)
            .bind(pagination.limit)
            .bind(offset)
            .fetch_all(self.db.get_read_pool())
    ...
    pub async fn get_account_locks(
        ...
        let token_lock_future = sqlx::query_as::<_, TokenLockInfoRaw>(&query)
            .bind(account_id)
            .bind(now)
            .bind(pagination.limit)
            .bind(offset)
    ...
    pub async fn get_account_withdrawable_lock(
        ...
        let count_future = sqlx::query(withdrawable_lock_count_query)
            .bind(account_id)
            .bind(now)
            .fetch_one(self.db.get_read_pool());
