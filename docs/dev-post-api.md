...
```json
{
  "rankings": [
    {
      "rank": 1,
      "token": { "token_id": "0x…", "name": "…", "symbol": "…", "image_uri": "https://…", "market_cap": "15875" },
      "total_likes": 900,
      "post_count": 12,
...
**코인당 하루(UTC) 3건 제한**: 같은 `token_id`로는 UTC 달력 기준 하루 최대 3건까지 작성할 수
있습니다(자정 리셋). 초과 시 `429` + `Retry-After` 헤더(다음 UTC 자정까지 남은 초)가 반환됩니다.
같은 날 글을 삭제해도 그날의 쿼터는 돌아오지 않습니다 — 내용 수정은 `PATCH`를 사용하세요.
제한은 생성 CTE 안의 카운트 게이트로 강제되며, 극단적 동시 요청에서는 순간적으로 초과될 수
있습니다(스팸 제한 목적).
...
- `429`: 해당 코인이 오늘(UTC) 이미 글을 게시함 (`Retry-After`: 다음 UTC 자정까지 초)
- `500`: 내부 서버 에러
...
| `token` | 코인 요약 (`token_id`/`name`/`symbol`/`image_uri`/`market_cap`) — `market_cap`은 market×price 조인 기반 **USD 마켓캡** (= hype `market_cap_usd` 의미). **스케일 주의: 랭킹(`GET /dev-post/ranking`)은 사람이 읽는 USD 정수 문자열(소수점 버림, 예: `"15875"`), 피드/상세/트렌딩은 아직 raw ×10^18 스케일** — 후속에서 사람 단위로 통일 예정. market 행 없으면 `null`, quote USD 가격 없으면 `"0"`. 캐시 TTL(≤60s)만큼 지연 가능 |
...
interface TokenSummary {
  ...
  market_cap: string | null;  // USD 마켓캡. 랭킹=사람 단위 정수(버림, "15875"), 피드/상세/트렌딩=raw ×10^18 (후속 통일 예정). market 행 없으면 null, quote USD 가격 없으면 "0"
}
