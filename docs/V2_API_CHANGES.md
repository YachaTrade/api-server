...
| 엔드포인트 | `GET/POST /x/webhook` + `GET /x/healthz` 신규 — X Account Activity 웹훅 수신(CRC + 서명). `api_key_gate` 우회, 인증은 HMAC 서명. gift-bot 역할 이전 (상세: [`docs/features/gift-webhook.md`](./features/gift-webhook.md)) |
