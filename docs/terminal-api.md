...
> **시장 노출 정책**: `/pair`와 `/events`는 `DEX` 마켓만 제공합니다. `CURVE` 및 알 수 없는 시장 유형은 노출하지 않습니다. `/asset`, `/{token_address}`, `/latest-block`의 동작은 시장 유형과 무관합니다.
...
- `dexKey`: `nadswap`
- `feeBps`: 생략. `fee_config` 제거 후 총 수수료를 정확히 계산할 근거가 없으므로 추정값을 내리지 않음
- `pairId`: DEX 풀 주소
- `asset0Id`/`asset1Id`: 토큰 주소와 `market.quote_id`를 소문자 오름차순으로 정렬
- `priceNative`: 정렬된 `asset0`를 `asset1` 단위로 표현한 가격
- 토큰과 quote 자산의 amount/reserve는 모두 18 decimals로 처리
...
순)으로 폴백하므로, 인덱싱된 자산은 노드 장애 중에도 404가 나지 않습니다.
...
| `asset.totalSupply` | string? | 온체인 총 공급량(십진화); RPC 실패 시 nadfun 토큰은 `1000000000`으로 폴백 |
...
거래쌍(Pair) 정보를 조회합니다. 쿼리 `id`는 **pair ID(= DEX pool 주소, `/events`가 내리는 `pairId`와 동일)**이며, 해당 pool의 `market_type`이 `DEX`인 경우에만 페어를 반환합니다. `CURVE` 또는 알 수 없는 시장 유형은 `404`로 처리됩니다.
...
| 파라미터 | 타입 | 필수 | 설명 |
|----------|------|------|------|
| `id` | string | O | DEX pool/pair 주소 |

#### 응답 (DEX 예시, quote = LVMON)
```json
{
  "pair": {
...
> 위 예시는 DEX(`nadswap`) 페어로, quote가 LVMON(`0xBe3f...`)이고 토큰 주소(`0xff00...`)가 더 큰 경우입니다. 소문자 비교에서 LVMON이 작으므로 LVMON이 `asset0`이 됩니다. `feeBps`는 총 수수료를 정확히 알 수 없어 생략됩니다.

#### 필드 설명

| 필드 | 타입 | 설명 |
|------|------|------|
| `pair.id` | string | DEX 풀 주소 (위 pairId 매핑 표 참고) |
| `pair.dexKey` | string | DEX 식별자 (`nadswap`) |
...
| `pair.feeBps` | number? | 현재 생략(총 수수료를 정확히 계산할 설정 없음) |
...
특정 블록 범위 내의 DEX 거래 이벤트(swap / join / exit)를 조회합니다. swap은 이벤트 자신의 `market_type`, join/exit는 연결된 현재 market을 기준으로 `DEX`만 반환하며 Curve 및 알 수 없는 시장 유형은 제외합니다.
...
#### 응답 (DEX swap + join 예시, quote = LVMON)
```json
{
  "events": [
    {
...
| `pairId` | string | DEX 이벤트의 풀 주소. 필터 기준은 이벤트 자신의 `market_type` |
...
2. **`/pair`는 DEX 현재 페어만 반환**: 현재 시장이 `DEX`인 토큰의 현재 페어 하나만 반환하며 Curve 토큰은 `404`입니다.
3. **Curve 이벤트 미노출**: `/events`는 `DEX` swap/join/exit만 반환하므로 졸업 전 Curve 거래 이력은 포함하지 않습니다.

---

## TypeScript Interfaces

> 아래 타입은 현재 `CURVE`/`DEX` 단일 세대 응답 스키마입니다.
